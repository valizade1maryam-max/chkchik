...
class BoundCamera(
    ...
    /** CONTROL_AF_MODE to restore when leaving manual focus (device's own normal auto mode). */
    private val autoFocusMode: Int = CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE,
) {
    ...
    /** Clears any manual-focus override, returning the lens to normal auto-focus. This
     *  explicitly re-asserts an auto AF mode instead of calling the interop's no-argument
     *  `clearCaptureRequestOptions()`, which would also wipe out any active white-balance
     *  override (it clears every interop key at once, not just the ones passed in). */
    @OptIn(ExperimentalCamera2Interop::class)
    fun clearManualFocus() {
        try {
            Camera2CameraControl.from(camera.cameraControl).addCaptureRequestOptions(
                CaptureRequestOptions.Builder()
                    .setCaptureRequestOption(CaptureRequest.CONTROL_AF_MODE, autoFocusMode)
                    .build()
            )
        } catch (e: Exception) {
            // ignore
...
/**
 * The device's own normal auto-focus mode for [info] (continuous picture when offered, else
 * plain auto, else whatever non-OFF mode it does report) - used to return to auto-focus after
 * a manual-focus override without touching any other active Camera2 interop option.
 */
private fun defaultAutoFocusMode(info: CameraInfo): Int {
    return try {
        val afModes = Camera2CameraInfo.from(info)
            .getCameraCharacteristic(CameraCharacteristics.CONTROL_AF_AVAILABLE_MODES)
        when {
            afModes == null -> CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE
            ...
            else -> afModes.firstOrNull { it != CameraCharacteristics.CONTROL_AF_MODE_OFF }
                ?: CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE
        }
    } catch (e: Exception) {
        CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE
 ...
 * [poseAnalyzer] (Stage 11), if given, is bound as a third `ImageAnalysis` use case alongside
 * preview/capture, on its own background thread - so live pose detection never blocks the
 * preview or the shutter. If a device rejects the three-use-case combination, the bind is
 * retried once without it so the camera still comes up (silently - live pose is a best-effort
 * add-on, never a reason the camera itself should fail).
 *
...
fun CameraPreview(
    ...
    onCameraError: () -> Unit,
    poseAnalyzer: ImageAnalysis.Analyzer? = null,
    ...
    val currentPoseAnalyzer by rememberUpdatedState(poseAnalyzer)
    ...
    // Dedicated background thread for pose analysis (Stage 11), separate from CameraX's own
    // internal threads, so a slow analysis run can never stall preview/capture. Lives as long
    // as this composable, independent of individual rebinds below.
    ...
    DisposableEffect(lifecycleOwner, previewView, imageCapture, lensFacing, lensOptionId) {
        ...
        providerFuture.addListener({
            ...
            try {
                ...
                // Stage 11: optional third use case for live pose detection. STRATEGY_KEEP_ONLY_LATEST
                // drops any frame the analyzer hasn't finished with yet, instead of queueing frames up -
                // exactly what a real-time, best-effort analysis (never blocking the camera) needs.
                val analyzer = currentPoseAnalyzer
                var analysis = analyzer?.let {
                ...
                fun useCases(): Array<UseCase> =
                    listOfNotNull(preview, imageCapture, analysis).toTypedArray()
                ...
                val camera = try {
                    ...
                    if (requestedLensId != null) {
                        try {
                            ...
                            // Still failing: the third (analysis) use case may be the culprit on
                            // this device - retry once more without it so the camera still works.
                            analysis = null
                ...
                currentOnCameraChanged(
                    BoundCamera(
                        ...
                        defaultAutoFocusMode(camera.cameraInfo)
                    )
                )
            } catch (e: Exception) {
